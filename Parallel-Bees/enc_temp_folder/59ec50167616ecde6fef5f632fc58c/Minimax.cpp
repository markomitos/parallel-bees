...
string* Minimax::FindBestMove()
{
    ...
    for (int i = 0; i < 6; i++) {
        ...
        long result = MiniMax(numeric_limits<long>::min(), numeric_limits<long>::max(), depth, opponentPlayer, currentState) + evaluate -> Eval(currentState, ourPlayer); //bilo okej kada smo ovde stavili bez -1 a u global 2
        ...
        long* evalTile = EvaluateTileNumber(stoi(moves[i][0]), stoi(moves[i][1]), moves[i][2], currentState, ourPlayer, depth); //mozda i ovde da ide -1 kod depth
